...
class GroundwaterGUI(QWidget):
    ...
    def manual_opt(self):
        try:
            ...
            conc_ = pat_model(*rates)
            self.update_concentration_plot(conc_,mode='manual')
            self.log(f"Manual optimization completed with rates: {rates}")
    ...
    def auto_opt(self):
        try:
            ...
            best_pumping, final_cost, history = pso_pat(pump_treat_objective, lb, ub, max_iter=3)
    ...
    def update_concentration_plot(self, conc_, mode=None, history=None):
        ...
        if mode == 'manual':
            ...
            for mw in monitoring_wells_xy:
                ax.scatter(mw[1], mw[2], color='blue', marker='s', label=mw[0], s=80)
            ...
            # Plot Monitoring Wells (Monitoring Wells are marked with 'bs' - blue squares)
            for mw in monitoring_wells_xy:
                ax1.scatter(mw[1], mw[2], color='blue', marker='s', label=mw[0], s=80)
