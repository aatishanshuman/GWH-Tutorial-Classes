...
#%%
ibound_path='D:/Courses Taught/Groundwater Hydrology/GWH Tutorial/Remediation Problems/iboundProblem2.csv'
strt_path='D:/Courses Taught/Groundwater Hydrology/GWH Tutorial/Remediation Problems/startingheadsProblem2.csv'
icbund_path='D:/Courses Taught/Groundwater Hydrology/GWH Tutorial/Remediation Problems/icbundProblem2.csv'
...
def pump_treat_objective(Q):
    Q1, Q2, Q3 = Q

    # Run the model with the given pumping rates
    conc = pat_model(Q1, Q2, Q3)
    ...
    treatment_threshold = 0.05  # ppm
    penalty_weight = 1e4       # scale for penalty
    penalty = sum([(max(0, c - treatment_threshold))**2 for c in conc_vals]) * penalty_weight

    pumping_cost = abs(Q1) + abs(Q2) + abs(Q3)
...
def pso_pat(objective, lb, ub, num_particles=5, max_iter=2, w=0.5, c1=1.5, c2=1.5):
